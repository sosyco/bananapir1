...
Banana Pi R1, Lamobo R1, Banana Pi Router or BPi-R1
...
sudo mkimage -A arm -O linux -T script -C none -a 0 -e 0 -n "BananPIR1 boot script" -d boot.cmd.sd-card boot/boot.scr
sudo umount boot
sudo umount root
...
sudo mkimage -A arm -O linux -T script -C none -a 0 -e 0 -n "BananPIR1 boot script" -d boot.cmd.sata /boot/boot.scr
reboot
```
...
# todo
- network-switch-configuration (lan1@eth0 -lan4@eth0)
